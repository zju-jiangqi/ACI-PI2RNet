...
def paired_paths_from_lmdb3(folders, keys):
    ...
    assert len(keys) == 3, (
        'The len of keys should be 2 with [input_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder, mq_folder = folders
    input_key, gt_key, mq_key = keys
    ...
    with open(osp.join(gt_folder, 'meta_info.txt')) as fin:
        gt_lmdb_keys = [line.split('.')[0] for line in fin]
    with open(osp.join(mq_folder, 'meta_info.txt')) as fin:
        mq_lmdb_keys = [line.split('.')[0] for line in fin]
    if set(input_lmdb_keys) != set(gt_lmdb_keys):
        raise ValueError(
            f'Keys in {input_key}_folder and {gt_key}_folder are different.')
    else:
        ...
        for lmdb_key in sorted(input_lmdb_keys):
            paths.append(
                dict([(f'{input_key}_path', lmdb_key),
                      (f'{gt_key}_path', lmdb_key),
                      (f'{mq_key}_path', lmdb_key)]))
        return paths


def paired_paths_from_meta_info_file3(folders, keys, meta_info_file,
                                     filename_tmpl):
    """Generate paired paths from an meta information file.
    ...
    for gt_name in gt_names:
        ...
        input_path = osp.join(input_folder, input_name)
        gt_path = osp.join(gt_folder, gt_name)
        mq_path = osp.join(mq_folder, gt_name)
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path),
                  (f'{mq_key}_path', mq_path)]))
    return paths
...
def paired_paths_from_folder3(folders, keys, filename_tmpl):
    ...
    input_paths = list(scandir(input_folder))
    gt_paths = list(scandir(gt_folder))
    mq_paths = list(scandir(mq_folder))
    assert len(input_paths) == len(gt_paths), (
        f'{input_key} and {gt_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(gt_paths)}.')
    paths = []
