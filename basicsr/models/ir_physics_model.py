...
class Image_Restoration_Physics_Model(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(Image_Restoration_Physics_Model, self).__init__(opt)
    ...
    def grids(self):
        ...
        self.original_psfsize = self.psf.size()
        assert b == 1
        ...
        while i < h and not last_i:
            ...
            while j < w and not last_j:
                ...
                for trans_idx in range(self.opt['val'].get('trans_num', 1)):
                    ...
                    psfparts.append(self.transpose(self.psf[:, :, i:i + crop_size, j:j + crop_size], trans_idx))
        ...
        self.origin_psf = self.psf
        self.lq = torch.cat(parts, dim=0)
        self.psf = torch.cat(psfparts, dim=0)
    ...
    def optimize_parameters(self, current_iter):
        ...
        preds, psf_preds = self.net_g(self.lq, self.psf)
        if not isinstance(preds, list):
            preds = [preds]
            psf_preds = [psf_preds]

        self.output = preds[-1]
        self.psfout = psf_preds[-1]
        ...
        if self.cri_pix:
            l_pix = 0.
            for pred, psf_pred in zip(preds, psf_preds):
                l_pix += self.cri_pix(pred,  psf_pred, self.gt)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                pred, xxx = self.net_g(self.lq[i:j, :, :, :], self.psf[i:j, :, :, :])
    ...
    def single_image_inference(self, img, psf, save_path):
        self.feed_data(data={'lq': img.unsqueeze(dim=0),'psf':psf.unsqueeze(dim=0)})
